...
NO_TEX_SUPSUB_PATTERN = re.compile(
    "(\{\S+\}\^\{\S+\}_\{\S+\})|(\{\S+\}\^\{\S+\})|(\{\S+\}_\{\S+\})|(\S_\{\S+\}\^\{\S+\})|(\S\^\{\S+\})|(\S_\{\S+\})"
)  # 不带$$符号的上下标
NO_TEX_SYMB_PATTERN = re.compile("(\\\\[a-zA-Z]+)")  # 带$$符号的特殊称号
NO_TEX_SYMB_SUPSUB_PATTERN = re.compile(
    "(\\\\[\w]+(\^\{[ -~]+\})?(_\{[ -~]+\}))|(\\\\[\w]+(_\{[ -~]+\})?(\^\{[ -~]+\}))"
)  # 不带$$符号的特殊符号有上下标
TEX_SUPSUB_PATTERN = re.compile(
    "(\$\S*\^?\S*_\S*\$)|(\$\S*_?\S*\^\S*\$)|(\\\(\S*_?\S*\^\S*\\\))|(\\\(\S*\^?\S*_\S*\\\))"
)  # 带$$符号的上下标
TEX_SYMB_PATTERN = re.compile("(\$\\\\[\w]+\$)|(\\\(\\\\[\w]+\\\))")  # 带$$符号的特殊称号
TEX_SYMB_SUPSUB_PATTERN = re.compile(
    "(\$\\\\[\w]+(\^\{[ -~]+\})?(_\{[ -~]+\})\$)|(\$\\\\[\w]+(_\{[ -~]+\})?(\^\{[ -~]+\})\$)|(\\\(\\\\[\w]+(\^\{[ -~]+\})?(_\{[ -~]+\})\\\))|(\\\(\\\\[\w]+(_\{[ -~]+\})?(\^\{[ -~]+\})\\\))"
)  # 带$$符号的特殊符号有上下标
